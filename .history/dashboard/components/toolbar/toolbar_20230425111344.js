...
class Toolbar {
  ...
  #AMPanel; // holds the application monitoring panel
  ...
  #bondToDom() {
    setTimeout(() => {
      ...
      this.#AMPanel = document.getElementById('7h0d4tuyhq');
  ...
  setListeners() {
    ...
    if (this.#SMPanel) {
      this.#SMPanel.addEventListener('click', () => {
        this.#openRoute('server_monitoring');
      });
    }
    if (this.#SLPanel) {
      this.#SLPanel.addEventListener('click', () => {
        this.#openRoute('server_logging');
      });
    }
    if (this.#SRPanel) {
      this.#SRPanel.addEventListener('click', () => {
        this.#openRoute('server_resources');
      });
    }
    if (this.#AMPanel) {
      this.#AMPanel.addEventListener('click', () => {
        this.#openRoute('app_monitoring');
      });
    }
    if (this.#ALPanel) {
      this.#ALPanel.addEventListener('click', () => {
        this.#openRoute('app_logging');
      });
    }
    if (this.#ARPanel) {
      this.#ARPanel.addEventListener('click', () => {
        this.#openRoute('app_resources');
      });
    }
  }
